...
import '../../api_key.dart';
import '../itemsScreens/food_seller_items_screen.dart';
...
class _BrandsUiDesignWidgetState extends State<BrandsUiDesignWidget> {
  ...
  Future<void> deleteBrand(String brandUniqueID, String thumbnailUrl) async {
    //Uri.parse("${API.foodSellercurrentSellerBrandView}?sellerID=$sellerID")
    var url = Uri.parse(
        "${API.foodSellerDeleteBrand}?brandUniqueID=$brandUniqueID&uid=$sellerID&thumbnailUrl=$thumbnailUrl");
        ...
        child: Padding(
          ...
          child: SizedBox(
            ...
            child: Column(
              children: [
                Image.network(
                  API.foodSellerBrandImage + (widget.model!.thumbnailUrl ?? ''),
                  height: 220,
                  fit: BoxFit.cover,
                ),
                const SizedBox(
